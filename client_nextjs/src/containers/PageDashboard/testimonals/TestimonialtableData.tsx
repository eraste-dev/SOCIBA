...
const TestimonialtableData: FC<TestimonialtableDataProps> = ({ rows }) => {
    ...
    const activeTestimony = (row: ITestimonial) => {
        const payload: ITestimonial = { ...row, validated: !row.validated, user_id: row.user?.id ?? 0 };
        dispatch(saveTestimonial(payload))
    }

    const deleteTestimony = (row: ITestimonial) => {
        const payload: ITestimonial = { ...row, validated: false, user_id: row.user?.id ?? 0 };
        // dispatch(saveTestimonial(payload))
    }
    ...
    return (
        <Paper sx={{ width: "100%", overflow: "hidden" }}>
                                ...
                                return (
                                    <React.Fragment key={id}>
                                        <TableRow hover role="checkbox" tabIndex={-1} key={id}>
                                            ...
                                            <TableCell>
                                                <div dangerouslySetInnerHTML={{ __html: shortMessage(message) ?? "" }} />
                                            </TableCell>
            ...
            />
            <SingleTestimonialDetail open={open} handleClose={() => setopen(false)} row={rowSelected} />
        </Paper>
    );
};

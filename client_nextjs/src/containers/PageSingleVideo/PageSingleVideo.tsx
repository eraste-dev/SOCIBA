...
const PageSingleVideo: FC<PageSingleVideoProps> = ({ className = "" }) => {
		...
		return (
			<div className="bg-neutral-800 rounded-3xl overflow-hidden border-4 border-white dark:border-neutral-700 shadow-2xl">
				{isSafariBrowser() && SINGLE_VIDEO.featuredImage && !isPlay && (
					<div className="absolute inset-0 z-10 cursor-pointer rounded-[18px] overflow-hidden" onClick={() => setIsPlay(true)}>
						<NcImage src={SINGLE_VIDEO.featuredImage} containerClassName="absolute inset-0" />
		...
		return (
			<div className={`nc-SingleHeader ${className}`}>
				<div className="space-y-5 dark text-neutral-100">
					...
					<SingleTitle mainClass="text-neutral-900 font-semibold text-3xl md:!leading-[120%] dark:text-neutral-100" title={title} />
	...
	return (
		<>
			<div className={`nc-PageSingleVideo  ${className}`} data-nc-id="PageSingleVideo">
				{/* SINGLE HEADER */}
				<header className="container relative py-14 lg:py-20 flex flex-col lg:flex-row lg:items-center">
					...
					<div className="relative lg:w-8/12 flex-shrink-0">
						<div className="aspect-w-16 aspect-h-16 sm:aspect-h-9 ">{renderMainVideo()}</div>
					</div>
				</header>

				{/* SINGLE MAIN CONTENT */}
				<div className="container mt-12">{/* <SingleContent data={SINGLE_VIDEO} /> */}</div>

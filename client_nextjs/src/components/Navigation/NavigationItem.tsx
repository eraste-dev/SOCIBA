	...
	const renderMegaMenu = (menu: NavItemType) => {
		...
		return (
			...
			>
				{() => (
					<>
						<Popover.Button as={Fragment}>{renderMainItem(menu)}</Popover.Button>
						<Transition
							as={Fragment}
							show={isHover}
						...
						>
							<Popover.Panel
								static
								className={`will-change-transform sub-menu absolute transform z-10 w-screen max-w-max px-4 pt-3 sm:px-0 lg:max-w-max ${classPanel}`}
							>
	...
	const renderMainItem = (item: NavItemType) => {
		...
		return item.targetBlank ? (
			...
			<NavLink
				...
				strict
				rel="noopener noreferrer"
				className="inline-flex items-center text-sm font-normal text-neutral-700 dark:text-neutral-300 py-1 px-4 xl:px-5 rounded-full hover:text-neutral-900 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:hover:text-neutral-200"
				to={{
					pathname: item.href || undefined,
				}}
				onClick={() => onClickItem()}

...
const PostCardMetaV2: FC<PostCardMetaV2Props> = ({ className = "leading-none", meta, hiddenAvatar = false, size = "normal" }) => {
	const { date, author, title } = meta;

	if (!author || (author && !author.href)) return null;
	...
	return (
		...
		>
			<Link to={author.href ? author.href : "#"} className="relative flex items-center space-x-2">
				{!hiddenAvatar && (
					<Avatar
						radius="rounded-full"
				...
				<div>
					<h2 className={`block font-semibold ${size === "normal" ? "text-base" : "text-lg"}`}>

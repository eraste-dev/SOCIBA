...
import { convertPayloadToFormData, mapIProductToProductRequest } from 'containers/PageDashboard/Posts/posts.constantes';
...
export const ProductCard13 = ({ setIsHover, row, isHover }: ProductCard13Props) => {
  ...
  const { id, title, description, category, status, images, videos, location, unlisted_city, location_description, price, periodicity, updated_at } = row;

  const handleChangeStatus = (row: IProduct, status: STATUS_LABEL) => {
    console.log('handleChangeStatus', row);
    const formData: FormData = convertPayloadToFormData(mapIProductToProductRequest({ ...row, status: status }));
  ...
  return (
    <div className="grid grid-cols-2  items-center p-2 cursor-pointer  ">
        ...
        >
          <div className="flex items-center post-image-container mr-2">
            <div className="h-[300px] w-full">
              <PostFeaturedMedia post={row} isHover={isHover} showOnlyVideo={canShowOnlyVideo()} />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="w-full">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="mt-2 text-xs font-semibold text-secondary-900 dark:text-neutral-100 ">
              ...
              {!location || (location && !location.name && unlisted_city) ? `Ville : ${unlisted_city}` : null}
              <br />
          ...
          <div>
            ...
            {row && row.author && row.author.href && <PostPropertyCardMetaV2 meta={row} />}
          </div>
        </div>
      </div>
      ...
      <div className="col-span-2 sm:col-span-1">
        {user && isAdmin(user) ? (
          <ChangeProductType lists={LIST_STATUS} selectedIndex={LIST_STATUS.findIndex((item) => item.name === row.status)} handleChange={(row: IProduct, status: STATUS_LABEL) => handleChangeStatus(row, status)} row={row} />
        ) : (

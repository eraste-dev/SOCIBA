...
import { initProductState, updateUser } from "app/axios/api.action";
...
const UserManagementTable: FC<UserManagementTableProps> = ({ rows, handleChangeView }) => {
	...
	const getStatus = (status: STATUS_TEXT) => {
		...
		switch (status) {
			...
			case "DELETED":
				return <span className={`${className} bg-red-500`}>Supprimé(e)</span>;

			case "ACTIVE":
				return <span className={`${className} bg-green-500`}>Actif</span>;
				...
				<Table stickyHeader aria-label="sticky table">
					...
					<TableBody>
						{rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
							const { id, name, last_name, phone, phone_whatsapp, type, status, count_products } = row;
							return (
								<TableRow hover role="checkbox" tabIndex={-1} key={id}>
									...
									<TableCell>
										...
										,{phone_whatsapp && <br />}
										{phone_whatsapp && (
											<a href={`https://wa.me/${phone_whatsapp}`} target="_blank" rel="noreferrer">
									...
									<TableCell>
										...
										<ChangeUserType
											lists={LIST_TYPE}
											selectedIndex={LIST_TYPE.findIndex((item) => item.name === type)}
									...
									<TableCell>
										...
										<ChangeUserType
											lists={LIST_STATUS}
											selectedIndex={LIST_STATUS.findIndex((item) => item.name === status)}
											handleChange={handleChangeType}
											row={row}
										/>
									</TableCell>

...
export interface SectionFormSugestion {
  name: string;
  email: string;
  message: string;
}
...
export interface SectionFormSugestionProps {
  className?: string;
}

const SectionFormSugestion: FC<SectionFormSugestionProps> = ({ className = '' }) => {
  ...
  } = useForm<SectionFormSugestion>();

  const onSubmit = (data: SectionFormSugestion) => {
      ...
      {isSubmitted && success ? (
        <div className="flex justify-center">
          <div className="w-2/3 flex flex-col items-center justify-center  ">
            <h1 className="text-3xl font-bold text-primary-800 text-center mb-5">Votre message a bien été envoyé</h1>
            <ButtonPrimary onClick={handleClickReset}>Contacter nous à nouveau</ButtonPrimary>
          </div>
        </div>
      ) : null}

      {!isSubmitted || !success ? (
        <div className="grid gap-2 grid-cols-3 mt-0 " style={{ marginTop: '10px !important' }}>
            ...
            <div className="w-full">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-6">
                <label className="block">
                  ...
                  <Input placeholder="Exemple Franck" autoComplete="on" type="text" className="mt-1" {...register('name')} />
                </label>

                <label className="block">
                  <Label>Email</Label>
                  <Input type="email" placeholder="franck@gmail.com" autoComplete="on" className="mt-1" {...register('email')} />
                </label>

                <label className="block">
                  <Label>Message</Label>
                  <Textarea className="mt-1" rows={6} {...register('message')} autoComplete="on" />
                </label>
...
export default SectionFormSugestion;
